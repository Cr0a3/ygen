        ...
        for (name, block) in body {
            ...
            for node in &mut block.body {
                ...
                if let Some(node) = any.downcast_ref::<Return<Type>>() {
                    ...
                    self.analiyze_assign_var(&mut vars, node, loc.to_owned())?;
                } else if let Some(node) = any.downcast_ref::<Assign<Var, Type>>() {
                    self.analiyze_assign_type(&mut vars, node, loc.to_owned())?;
                    ...
                    self.analyze_neg(&mut vars, node, loc.to_owned())?;
                } else if let Some(node) = any.downcast_ref::<Select<Var, Var>>() {
                    self.analyze_selectvv(&mut vars, node, loc.to_owned())?;
                } else if let Some(node) = any.downcast_ref::<Select<Var, Type>>() {
                    self.analyze_selectvt(&mut vars, node, loc.to_owned())?;
                } else if let Some(node) = any.downcast_ref::<Select<Type, Var>>() {
                    self.analyze_selecttv(&mut vars, node, loc.to_owned())?;
                } else if let Some(node) = any.downcast_ref::<Select<Type, Type>>() {
                    self.analyze_selecttt(&mut vars, node, loc.to_owned())?;
                } else if let Some(node) = any.downcast_ref::<GetElemPtr>() {
                    self.analyze_getelemptr(&mut vars, node, loc.to_owned())?;
                } else {
